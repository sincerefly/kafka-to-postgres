...
async fn save_data(mut pool: &PgPool, sql_string: String) -> anyhow::Result<u64> {
...
async fn main() -> anyhow::Result<()> {
    ...
    // 从环境变量获取配置信息,不设置程序将退出
    let database_url = &env::var("DATABASE_URL").context("`DATABASE_URL` must be set to run this consumer")?;
